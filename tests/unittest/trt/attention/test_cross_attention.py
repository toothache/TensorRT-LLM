...
class TestPluginCrossAttention(unittest.TestCase):
    ...
    @parameterized.expand([("float16", True, ContextFMHAType.enabled),],
                          name_func=unittest_name_func)
    def test_plugin_cross_attention(self, dtype: str, remove_input_padding: bool,
        ...
        if use_random_data:
            ...
            # cross_k = torch.ones(
            #     cross_k_v_shape, dtype=str_dtype_to_torch(dtype), device="cuda")
            cross_k = torch.randn((max_kv_input_len, num_kv_heads * head_size), dtype=str_dtype_to_torch(dtype), device="cuda")

            # cross_k = torch.randn(head_size * num_kv_heads, dtype=str_dtype_to_torch(
            #     dtype), device="cuda").reshape(1, 1, -1).tile(max_batch_size, max_kv_input_len, 1)
        ...
        cross_attention_mask = torch.ones((max_batch_size * max_input_len, max_kv_input_len), dtype=torch.bool, device="cuda")
        ...
        mean_diff = torch.mean(torch.abs(output - out_torch))
        max_diff = torch.max(torch.abs(output - out_torch))
